...
%% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Protocol Exp 1
    ...
    ynow = 600;
    if 1
        xvec = [xnow+300 xnow+410 xnow+520+30 xnow+630+30 xnow+740+30 xnow+850+30];
        yvec = ynow:-110:-1000;
        m = {'adaptive','kadaptive','yadaptive','kyadaptive'};
        m2 = {sprintf('Adaptive smoothing\n(pixelwise)'),sprintf('Adaptive smoothing\n(convolution)'),sprintf('Adaptive binning\n(pixelwise)'),sprintf('Adaptive binning\n(convolution)')};
        ...
        override_maps = 0;
        fname = [fig_dir 'adapt_comparison_maps.mat'];
            ...
            for yy = 1:nplots
                ...
                ax = axes('Units','pixels','Position',[xvec(xx) yvec(yy) psiz psiz]);    
                    if mem
                        ...
                        rmset.method = m{xx};
                        rmset.binsize = bnow(xx);
                        rmset.ssigma = snow(xx);
        ...
        ax1 = axes('Units','pixels','Position',[ax.Position(1)-psiz*3.6 ax.Position(2)-110 psiz*2+10 80]);
            v1 = mean([maps_for_plotting{1,:,2}],'omitnan');
            v2 = mean([maps_for_plotting{2,:,2}],'omitnan');         
            bar(1:2,[v1 v2],0.5,'k');
            
            ax1.XTick = [1:2];
            ax1.XTickLabel = {'Pixelwise','Convolution'};
            ax1.XLim = [0.5 2.5];
            ax1.YLim = [0 15];
            ylabel(sprintf('Computation time (s)'))
            box off
            
        ax2 = axes('Units','pixels','Position',[ax.Position(1)-psiz*1.1 ax1.Position(2) ax1.Position(3) ax1.Position(4)]);
            v1 = mean([maps_for_plotting{3,:,2}],'omitnan');
            v2 = mean([maps_for_plotting{4,:,2}],'omitnan');          
            bar(1:2,[v1 v2],0.5,'k');
            
            ax2.XTick = [1:2];
            ax2.XTickLabel = {'Pixelwise','Convolution'};
            ax2.XLim = [0.5 2.5];
            ax2.YLim = [0 2.5];
            ylabel(sprintf('Computation time (s)'))
            box off            
