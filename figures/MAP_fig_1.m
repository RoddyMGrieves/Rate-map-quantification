...
%% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Protocol Exp 1
    ...
    axe = axes('Units','pixels','Position',[xnow ynow 180 180]); 
        ...
        text(0.5,-0.11,'1200 mm','Units','normalized','FontSize',font_siz,'HorizontalAl','center')
        text(-0.12,0.5,'1200 mm','Units','normalized','FontSize',font_siz,'HorizontalAl','center','Rotation',90)
    ...
    axp = axes('Units','pixels','Position',[xnow ynow 180 180]);  
        ah = add_panel_title('B',sprintf('Place cell firing\nprobability map'),'yoffset',5,'xoffset',0,'width',400);  
        ...
        text(0,-0.1,sprintf('Centroid = %d, %d\n%c_{x} = %.f, %c_{y} = %.f, %c_{xy} = %.f',cents(1),cents(2),963,sqrt(sigma(1)),963,sqrt(sigma(2)),963,sigma(3)),'Units','normalized','FontSize',font_siz,'HorizontalAl','left')

    ax = axes('Units','pixels','Position',[axp.Position(1)+190 axp.Position(2)+40 10 120]);
        x = 1:100;
        imagesc(x');
        colormap(gca,rmap_colormap)  
        axis xy off
    ...
    axs = axes('Units','pixels','Position',[xnow ynow 180 180]);   
        ...
        axis off xy
        daspect([1 1 1])
        text(0,1.05,'16 minutes','Units','normalized','FontSize',font_siz,'HorizontalAl','left');
    ...
    for i = 1:3
        axn = axes('Units','pixels','Position',[xnow+((i-1)*hbuff)+((i-1)*wid) ynow wid wid]);   
            if i==1
                ah = add_panel_title('D',sprintf('Simulated field sizes'),'yoffset',0,'xoffset',0,'width',400);                  
            end
            ...
            text(0,-0.1,sprintf('Centroid = %d, %d\n%c_{x} = %.f, %c_{y} = %.f, %c_{xy} = %.f',cents(1),cents(2),963,sqrt(sigma(1)),963,sqrt(sigma(2)),963,sigma(3)),'Units','normalized','FontSize',font_siz,'HorizontalAl','left')
            text(0,1,sprintf('Radius %d',i),'Units','normalized','FontSize',font_siz,'HorizontalAl','left','VerticalAl','bottom');            
            text(0,1,sprintf('Approx. 2%c radius: %d mm',963,round(fs2(i))),'Units','normalized','FontSize',font_siz,'HorizontalAl','left','Color','w','VerticalAl','top')
    ...
    for i = 1:3
        axn = axes('Units','pixels','Position',[xnow+((i-1)*hbuff)+((i-1)*wid) ynow wid wid]);   
            ...
            p1 = plot(pox,poy,'Color','k'); hold on;
            daspect([1 1 1])
            axis xy off  
            plot(epoly(:,1),epoly(:,2),'k','LineWidth',1.5);
            text(0,1.05,sprintf('Duration %d: %d minutes',i,durs(i)),'Units','normalized','FontSize',font_siz,'HorizontalAl','left');
